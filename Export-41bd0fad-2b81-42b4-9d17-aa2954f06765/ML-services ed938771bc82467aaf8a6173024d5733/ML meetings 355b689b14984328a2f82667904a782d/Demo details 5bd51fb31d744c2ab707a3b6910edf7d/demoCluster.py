...
if __name__ == "__main__":
    ...
    abnormal_duration = 5*60
    exp_loop_duration = 60
    eta_total_min = int(abnormal_duration / 60)

    abnormal_iter = int(abnormal_duration / exp_loop_duration)

    print(f"\nNumber of iterations for each state:\n"
        f"Abnormal state: {abnormal_iter} iteration(s)\n"
        f"ETA = {eta_total_min} min. {abnormal_duration - 60*eta_total_min} s.")
    ...
    while True:
        ...
        i += 1
        eta = abnormal_duration - i*exp_loop_duration
        eta_min = int(eta/60)
        if eta_min > 0:
