...
if __name__ == "__main__":
    ...
    normal_duration = 120*60
    exp_loop_duration = 10
    eta_total_min = int(normal_duration / 60)

    normal_iter = int(normal_duration / exp_loop_duration)

    print(f"\nNumber of iterations for each state:\n"
        f"Normal state: {normal_iter} iteration(s)\n"
        f"ETA = {eta_total_min} min. {normal_duration - 60*eta_total_min} s.")
    ...
    while True:
        ...
        i += 1
        eta = normal_duration - i*exp_loop_duration
        eta_min = int(eta/60)
        if eta_min > 0:
            eta_s = f"{eta_min} min. {eta - 60*eta_min} s."
        else:
            eta_s = f"{eta_min} min. {eta - 60*eta_min} s."
    ...
    print(f"\nFinished normal behavior.\n"
        f"[Time normal behavior: {normal_time}]\n"
    )
