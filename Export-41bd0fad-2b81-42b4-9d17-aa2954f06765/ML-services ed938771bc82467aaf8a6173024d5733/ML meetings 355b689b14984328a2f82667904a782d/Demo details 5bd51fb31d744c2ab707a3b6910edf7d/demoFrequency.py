...
if __name__ == "__main__":
    ...
    heavy_duration = 5*60
    exp_loop_duration = 11
    eta_total_min = int(heavy_duration / 60)

    heavy_iter = int(heavy_duration / exp_loop_duration)

    print(f"\nNumber of iterations for each state:\n"
        f"Heavy load state: {heavy_iter} iteration(s)\n"
        f"ETA = {eta_total_min} min. {heavy_duration - 60*eta_total_min} s.")
    ...
    while True:
        ...
        i += 1
        eta = heavy_duration - i*exp_loop_duration
        eta_min = int(eta/60)
        if eta_min > 0:
